...
文献调研整体 PRISMA 审计链路见 [Literature PRISMA Audit Flow](./literature-prisma-audit.md)。本文档重点说明 Research Brief 如何进入这条链路，以及偏好如何影响查询、筛选、证据表和最终报告。
...
需要注意：这只能保证偏好影响被记录和披露，不能保证自动 synthesis 已达到专家可用质量。当前系统仍可能因为 topic expansion 过宽、query 约束不足或 screening 语义判断偏弱而纳入低相关论文。
...
对于 AI4S、AI-Infra 等宽泛主题，建议额外人工抽查：

- 纳入论文是否同时命中核心概念和具体场景，而不是只命中领域词。
- `preference_scores.json` 的高分论文是否真的回答研究问题。
- `screening_log.json` 中排除比例是否异常偏低。
- `review_findings.json` 为空时，是否仍存在明显主题漂移或证据不足。
...
Research Brief 和 PRISMA artifact 链路提高的是可追溯性和可审计性，不等同于自动生成最终高质量综述。当前文献调研报告仍可能存在：

- 主题扩展过宽。
- 普通领域论文被误纳入。
- 缺少 full-text 证据。
- quality tier 主要基于 metadata 和摘要。
- synthesis 偏模板化。
- review agent 漏检主题漂移。

因此当前输出更适合作为“可审计调研草稿”和“后续人工/agent 精筛输入”，而不是直接作为最终可用报告。
